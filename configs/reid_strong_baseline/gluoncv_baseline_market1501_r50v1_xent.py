...
def get_config(task):
    ...
    class OptimizeParam:
        ...
        class schedule:
            begin_epoch = 0
            end_epoch = 120
            lr_iter = [25 * 400 * 32 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       50 * 400 * 32 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       75 * 400 * 32 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)]
            lr_factor = 0.1

        class warmup:
            type = "gradual"
            lr = 0.00035 / 32 * len(KvstoreParam.gpus) * KvstoreParam.batch_image / 100
            iter = 4 * 400
    ...
    class IOParam:
        ...
        from poi.core.image import (
            ...
            Norm2DImage, ConvertImageFromHWCToCHW, RenameRecord
        )
