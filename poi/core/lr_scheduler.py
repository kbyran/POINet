...
class WarmupMultiFactorScheduler(LRScheduler):
    def __init__(self, step, factor=1, warmup=False, warmup_type='constant', warmup_lr=0,
                 warmup_step=0, logger=None):
        super().__init__()
    ...
    def __call__(self, num_update):
        if self.warmup and num_update <= self.warmup_step:
            if self.warmup_type == 'constant':
                ...
                warmup_lr = (self.base_lr - self.warmup_lr) \
                    / self.warmup_step * num_update + self.warmup_lr
                return warmup_lr
...
class AdvancedLRScheduler(LRScheduler):
    ...
    def __init__(self, mode, base_lr=0.1, target_lr=0,
                 ...
                 baselr=None, targetlr=None, logger=None):
        super().__init__()
        ...
        if baselr is not None:
            logger.warn("baselr is deprecated. Please use base_lr.")
            if base_lr == 0.1:
                base_lr = baselr
        self.base_lr = base_lr
        if targetlr is not None:
            logger.warn("targetlr is deprecated. Please use target_lr.")
            if target_lr == 0:
                target_lr = targetlr
