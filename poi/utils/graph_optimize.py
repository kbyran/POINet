...
def merge_bn(symbol, args, auxs, symbol_only=False):
    ...
    for nid, node in enumerate(jnodes):
        ...
        if op_name == "null":
            ...
            gamma_name, beta_name, mmean_name, mvar_name = \
                gamma.name, beta.name, mmean.name, mvar.name
            ...
            if attrs["use_global_stats"] == "True" and node_op_map[e[0]][e[1]] == "Convolution":
                if not symbol_only:
                    if (mmean_name) not in auxs:
                        logging.info("Can not find {}, merge the symbol only".format(
                            node_name + "_moving_mean"))
                    else:
                        logging.info("Merging {}".format(node_name))
                        # modify beta before gamma since gamma is not depend on beta
                        args[beta_name] -= args[gamma_name] * \
                            auxs[mmean_name] / mx.nd.sqrt(eps + auxs[mvar_name])
                        args[gamma_name] /= mx.nd.sqrt(eps + auxs[mvar_name])
                        # expand for broadcasting
                        if args[gamma_name].ndim == 1:
                            args[gamma_name] = args[gamma_name].expand_dims(
                                axis=0).expand_dims(axis=-1).expand_dims(axis=-1)
                            args[beta_name] = args[beta_name].expand_dims(
                                axis=0).expand_dims(axis=-1).expand_dims(axis=-1)
                            auxs[mmean_name] = auxs[mmean_name].expand_dims(
                                axis=0).expand_dims(axis=-1).expand_dims(axis=-1)
                            auxs[mvar_name] = auxs[mvar_name].expand_dims(
                                axis=0).expand_dims(axis=-1).expand_dims(axis=-1)
                        # set mmean and mvar to identity to avoid fusing more than once
                        # in weight sharing
                        auxs[mmean_name][:] = 0.0
                        auxs[mvar_name][:] = 1.0
                ...
                res = mx.sym.broadcast_add(mx.sym.broadcast_mul(
                    data=children[0], scaler=gamma), beta)
            else:
...
if __name__ == "__main__":
    model = "logs/strong_baseline_market1501_r50v1_xent_tri_cent/checkpoint.json"
