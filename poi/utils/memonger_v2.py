...
def search_plan(sym, ntrial=6, type_dict=None, logger=None, **kwargs):
    ...
    for k in range(nbegin):
        ...
        logger.info("Search threshold=%d MB, cost=%d MB" % (threshold, cost))
    ...
    logger.info('Find best plan with threshold=%d, cost=%d MB' % (threshold, cost))
    return sym
...
def search_plan_to_layer(sym, layer_name=None, threshold=500, ntrial=6, type_dict=None,
                         logger=None, **kwargs):
    """Quickly heurestic search over possible plans to find good memory plan.
    ...
    for k in range(nbegin):
        info = {}
        sym = make_mirror_plan_to_layer(sym, layer_name=layer_name,
                                        threshold=threshold, plan_info=info, **kwargs)
        cost = get_cost(sym, type_dict, **kwargs)
        # save_size = info['save_size'] >> 20
        local_size = info['max_size'] >> 20
        guess = 300 * (k + 1)
    ...
    if step > 0:
        for k in range(ntrial):
            sym = make_mirror_plan_to_layer(
                sym, layer_name=layer_name, threshold=threshold, plan_info=info, **kwargs)
            cost = get_cost(sym, type_dict, **kwargs)
            logger.info("Search threshold=%d MB, cost=%d MB" % (threshold, cost))
            history.append((cost, threshold, sym))
            threshold += step
